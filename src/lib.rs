...
pub trait Game: Clone {
    ...
    /// the fitness of a computed action
    type Fitness: Fitness + Ord + Copy;
    ...
    fn execute(self, action: &Self::Action, player: &Self::Player) -> StepResult<Self>;

    /// Returns the resulting fitness after `action` is executed.
    /// The returned fitness is always from the perspective of `player`,
    /// even if the `player` is not active.
    ///
    ...
    fn look_ahead(&self, action: &Self::Action, player: &Self::Player) -> LookAheadResult<Self> {
        match self.clone().execute(action, player) {
            StepResult::Open(_state, fitness) => LookAheadResult::Open(fitness),
            StepResult::Terminated(fitness) => LookAheadResult::Terminated(fitness),
        }
    }
}

pub enum StepResult<T: Game> {
    Terminated(T::Fitness),
    Open(T, T::Fitness),
}

impl<T: Game> StepResult<T> {
    pub fn fitness(&self) -> T::Fitness {
        match self {
            StepResult::Open(_state, fitness) => *fitness,
            StepResult::Terminated(fitness) => *fitness,
        }
    }
}

impl<T: Game> Debug for StepResult<T>
where
    T: Debug,
    T::Fitness: Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            StepResult::Open(state, fitness) => write!(f, "Open({:?}, {:?})", state, fitness),
            StepResult::Terminated(fitness) => write!(f, "Terminated({:?})", fitness),
        }
    }
}

#[derive(Clone, Copy)]
pub enum LookAheadResult<T: Game> {
    Terminated(T::Fitness),
    Open(T::Fitness),
}

impl<T: Game> LookAheadResult<T> {
    pub fn fitness(&self) -> T::Fitness {
        match self {
            LookAheadResult::Open(fitness) => *fitness,
            LookAheadResult::Terminated(fitness) => *fitness,
        }
    }
}

impl<T: Game> Debug for LookAheadResult<T>
where
    T::Fitness: Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            LookAheadResult::Open(fitness) => write!(f, "Open({:?})", fitness),
            LookAheadResult::Terminated(fitness) => write!(f, "Terminated({:?})", fitness),
        }
    }
}
...
///
/// `is_upper_limit` should only return `true` in case there can not be a better state,
